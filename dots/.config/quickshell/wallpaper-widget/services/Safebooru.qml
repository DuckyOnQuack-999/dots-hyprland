...
QtObject {
    ...
    // Safebooru configuration
    readonly property var sourceConfig: Config.getSourceConfig("safebooru")
    ...
    readonly property string defaultTags: sourceConfig.defaultTags || "rating:general"
    ...
    // Safebooru API endpoints
    readonly property string baseUrl: "https://safebooru.org"
    readonly property string apiEndpoint: baseUrl + "/api/json/post/index"
    ...
    function buildSearchUrl(tags, nsfw, page) {
        ...
        // Add rating (SFW only by default)
        if (!allowNsfw && !nsfw) {
            params.push("rating=general")
        } else if (nsfw && apiKey && apiKey.length > 0) {
            // NSFW with API key - use general rating to avoid issues
            params.push("rating=general")
        }
    ...
    function ensureWallpaperTags(wallpaperId) {
        ...
        var url = baseUrl + "/post/view/" + wallpaperId + ".json"
